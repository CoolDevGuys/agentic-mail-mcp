...
- **Gmail** is wrapped in a ``LazyGmailGateway`` so tools register at startup even
  ...
  (or fails with a clear "run `gmail-mcp-server auth`" message).
...
from src.Intelligence.Infrastructure.InMemory.repositories import (
...
def _build_semantic_search(settings: Settings):
    ...
    try:
        from src.Search.Application.UseCases.semantic_search import (
            SemanticSearchUseCase,
        )
        from src.Search.Infrastructure.BGE.bge_embedding_gateway import (
            BgeEmbeddingGateway,
        )
