...
## Purpose
Define the `init` subcommand: an interactive wizard that guides a user through every configuration section, validates input, safely handles an existing configuration file, and writes a `.env` file that `Settings.from_env()` can load without error.
