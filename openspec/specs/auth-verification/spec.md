# auth-verification Specification

## Purpose
Verify that the server can authenticate to Gmail, both on demand via a `verify-auth` command and as a non-blocking preflight at HTTP startup, reporting actionable per-stage failures.
